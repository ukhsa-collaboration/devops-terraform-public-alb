...
Egress rules are derived from backend target group ports and explicit health check ports, then deduplicated by effective destination. By default they allow traffic to `0.0.0.0/0`, but you can restrict individual backends to specific security groups via `egress_security_group_ids`.
